...
func (d *dexterOIDC) createOauth2Config() error {
	...
	provider, err := oidc.NewProvider(ctx, "http://accounts.example.com")
	...
	d.Oauth2Config.Scopes = []string{oidc.ScopeOpenID, "profile", "email", "offline_access"}

	return nil
}

func (d *dexterOIDC) authUrl() string {
	return d.Oauth2Config.AuthCodeURL(d.state, oauth2.AccessTypeOffline)
}
...
func authCommand(cmd *cobra.Command, args []string) {
	...
	if err := OpenURL(oidcData.authUrl()); err != nil {
